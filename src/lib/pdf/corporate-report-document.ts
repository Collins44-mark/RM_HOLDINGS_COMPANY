...
const STATUS_FILL: Record<string, string> = {
  "In Stock": "0.910 0.965 0.925",
  "Low Stock": "1.000 0.957 0.898",
  Expired: "0.992 0.925 0.925",
  "Out of Stock": "0.992 0.925 0.925",
  "Expiring Soon": "1.000 0.957 0.898",
};

const STATUS_INK: Record<string, string> = {
  "In Stock": "0.22 0.48 0.35",
  "Low Stock": "0.62 0.42 0.12",
  Expired: "0.68 0.22 0.22",
  "Out of Stock": "0.68 0.22 0.22",
  "Expiring Soon": "0.62 0.42 0.12",
};
...
export type SectionTableOptions = {
...
/**
 * Shared corporate A4 report document for supermarket PDF exports.
 * Soft rounded sections, light headers, no dark bars / dashboard chrome.
 */
export class CorporateReportDocument {
  ...
  private paintStatusPill(x: number, y: number, width: number, height: number, value: string) {
    const label = pdfAscii(value);
    const fill = STATUS_FILL[label] ?? FILL_HEAD;
    const ink = STATUS_INK[label] ?? INK;
    const pillW = Math.min(width - 6, Math.max(48, pdfTextWidth(label, 7, true) + 14));
    const pillH = 11;
    const pillX = x + 4;
    const pillY = y + (height - pillH) / 2;
    this.page.ops.push(pdfFillRoundRect(pillX, pillY, pillW, pillH, 5, fill));
    this.page.ops.push(`${ink} rg`);
    this.page.ops.push(
      pdfText("F2", 7, pillX + (pillW - pdfTextWidth(label, 7, true)) / 2, pillY + 3, label),
    );
  }
      ...
      if (options.statusKey && col.key === options.statusKey) {
        ...
        this.page.ops.push(
          pdfAlignedText(
            ...
            ry + 4.2,
            col.width,
            pdfClip(value, Math.max(6, Math.floor(col.width / 3.5))),
            col.align ?? "left",
          ),
        );
      }
  ...
  private drawSectionChrome(
    ...
    this.setStroke(BORDER);
    this.page.ops.push(pdfStrokeRoundRect(boxX, boxBottom, boxW, sectionH, SECTION_RADIUS, 0.55));

    const titleY = boxTop - titleH;
    this.page.ops.push(
      pdfFillRoundRect(boxX + 0.4, titleY, boxW - 0.8, titleH, SECTION_RADIUS - 1, FILL_HEAD),
    );
    this.page.ops.push(
      `${FILL_HEAD} rg ${(boxX + 0.4).toFixed(2)} ${titleY.toFixed(2)} ${(boxW - 0.8).toFixed(2)} 5 re f`,
    );
    this.setInk(INK);
    this.page.ops.push(pdfText("F2", boxW > 300 ? 10 : 9, boxX + padX, titleY + 5.5, title));

    let contentTop = titleY - 2;
    if (subtitle) {
      this.setInk(MUTED);
      this.page.ops.push(pdfText("F1", 7.5, boxX + padX, titleY - 12, subtitle));
      contentTop = titleY - 18;
    }
    return { boxBottom, contentTop };
  }

  /** Single-page rounded section (summary / short tables). */
  addSectionTable(
    title: string,
    columns: TableColumn[],
    rows: Record<string, string>[],
    ...
    const titleH = 18;
    const subtitleH = options?.subtitle ? 14 : 0;
    const headerH = 15;
    const rowH = 16;
    const bodyRows = rows.length + (options?.totalRow ? 1 : 0);
    const bottomPad = 8;
    const tableW = CONTENT_W - PAD_X * 2;
    const sectionH = titleH + subtitleH + 4 + headerH + rowH * bodyRows + bottomPad;
    ...
    const { boxBottom, contentTop } = this.drawSectionChrome(
      MARGIN_X,
      boxTop,
      CONTENT_W,
      sectionH,
  ...
  /**
   * Long tables that may span pages. Each page chunk is a rounded section;
  ...
  addFlowingSectionTable(
    ...
    const rowH = 16;
    const bottomPad = 8;
    const topChrome = titleH + subtitleH + 4 + headerH + bottomPad;
    const tableW = CONTENT_W - PAD_X * 2;
    const scale = tableW / columns.reduce((sum, col) => sum + col.width, 0);
    const scaled = columns.map((col) => ({ ...col, width: col.width * scale }));
    ...
    while (index < rows.length) {
      const sectionLabel = firstChunk ? title : `${title} (continued)`;
      this.continuingSection = title;

      const available = this.page.y - MARGIN_BOTTOM;
      let rowsFit = Math.floor((available - topChrome - 12) / rowH);
      if (rowsFit < 1) {
        this.newPage(title);
        rowsFit = Math.floor((this.page.y - MARGIN_BOTTOM - topChrome - 12) / rowH);
      }
      rowsFit = Math.max(1, rowsFit);

      const chunk = rows.slice(index, index + rowsFit);
      const isLast = index + chunk.length >= rows.length;
      const totalForChunk = isLast ? options?.totalRow : undefined;
      const chunkRows = chunk.length + (totalForChunk ? 1 : 0);
      const sectionH = titleH + (firstChunk ? subtitleH : 0) + 4 + headerH + rowH * chunkRows + bottomPad;

      if (this.page.y - sectionH - 12 < MARGIN_BOTTOM && index > 0) {
        this.newPage(title);
        continue;
      }
      ...
      const { boxBottom, contentTop } = this.drawSectionChrome(
        ...
        titleH,
        PAD_X,
        firstChunk ? options?.subtitle : undefined,
      );

      let y = this.paintTableHeader(
        MARGIN_X + PAD_X,
        contentTop,
        tableW,
        scaled,
      ...
      if (totalForChunk) {
        this.paintDataRow(MARGIN_X + PAD_X, y, tableW, scaled, totalForChunk, rowH, 8, {
          bold: true,
          tint: true,
          statusKey: options?.statusKey,
        });
  ...
  addTwoColumnSections(
    ...
    const render = (
      ...
      const { contentTop } = this.drawSectionChrome(
        x,
        startY,
        COL_W,
        sectionH,
...
/** @deprecated Use CorporateReportDocument — kept for Sales Report import compatibility */
export class CorporateSalesDocument extends CorporateReportDocument {}
